...
class StudentMap extends BaseMap {
    ...
    public function findAll($ofset = null, $limit = 30)
    {
        $res = $this->db->query("SELECT user.user_id, CONCAT(user.lastname,' ', user.firstname, ' ', user.patronymic) AS fio, user.birthday, "
                              . " gender.name AS gender, gruppa.name AS gruppa, student.num_zach AS num_zach, role.name AS role FROM user INNER JOIN student ON user.user_id=student.user_id "
